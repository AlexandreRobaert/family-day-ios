        ...
        AF.request("\(Configuration.URL_API)/familias/listar-membros", headers: header).validate().responseData { (data) in
            do {
                ...
                for item in json.arrayValue {
                    ...
                    let user = Usuario(id: item["_id"].stringValue, nome: item["nome"].stringValue, dataNascimento: dataNascimento, telefone: item["telefone"].stringValue, tipo: item["tipo"].stringValue, email: item["email"].stringValue, genero: genero, senha: "", idFamilia: Configuration.shared.idFamilia!)
                    users.append(user)
                }
                completion(users)
